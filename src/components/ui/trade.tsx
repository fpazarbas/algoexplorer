...
function ActiveNavLink({ href, title, isActive, className }: any) {
  ...
  return (
    ...
    >
      <span>{title}</span>
      {isActive && (
        <span className="absolute left-0 right-0 bottom-0 -z-[1] h-full w-full rounded-lg bg-brand shadow-large" />
      )}
    </ActiveLink>
  );
}
...
export default function Trade({ children }: React.PropsWithChildren<{}>) {
  ...
  return (
    <div className="pt-8 text-sm xl:pt-10">
      <div className="mx-auto w-full max-w-lg rounded-lg bg-white p-5 pt-4 dark:bg-dark xs:p-6 xs:pt-5">
        {/* <nav className="mb-5 min-h-[20px] border-b border-dashed border-gray-200 pb-4 uppercase tracking-wider dark:border-gray-700 xs:mb-6 xs:pb-5 xs:tracking-wide">
