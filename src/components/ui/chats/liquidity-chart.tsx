...
function getVestigePrice() {
  fetch('https://free-api.vestige.fi/asset/1035899249/prices/simple/7D')
    .then((res) => res.json())
    .then((json) => {
      const items = json;
      items.map((item: { timestamp: any; price: any }) => {
        item.price;
        item.timestamp;
        console.log(item.price);
        var priceData = new DataPrice(item.price, item.timestamp, 0, 1);
        prices.fill(priceData);
      });
